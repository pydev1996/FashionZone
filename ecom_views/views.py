...
def product_details(request, pn):
    product = get_object_or_404(Product, name=pn)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            review.save()
            return redirect('product_details', pn=product.name)
    else:
        form = ReviewForm()
...
def customer_home_view(request):
    # Fetch categories from Product model choices
    categories = [choice[0] for choice in models.Product.CATEGORY_CHOICES]
    
    # Get selected category from request parameters, if any
